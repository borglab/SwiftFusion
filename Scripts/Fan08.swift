...
/// Fan08: Raw Tracker, with Identity projection
struct Fan08: ParsableCommand {
  ...
  func run() {
    ...
    let (fig, track, gt) = runProbabilisticTracker(
      ...
      savePatchesIn: "Results/fan08"
    )

    /// Actual track v.s. ground truth track
    fig.savefig("Results/fan08/fan08_track\(trackId)_\(featureSize).pdf", bbox_inches: "tight")
    fig.savefig("Results/fan08/fan08_track\(trackId)_\(featureSize).png", bbox_inches: "tight")

    let json = JSONEncoder()
    json.outputFormatting = .prettyPrinted

    let track_data = try! json.encode(track)
    try! track_data.write(to: URL(fileURLWithPath: "Results/fan08/fan08_track_\(trackId)_\(featureSize).json"))

    let gt_data = try! json.encode(gt)
    try! gt_data.write(to: URL(fileURLWithPath: "Results/fan08/fan08_gt_\(trackId)_\(featureSize).json"))
  }
}
