...
public struct NNClassifier: Layer{
  ...
  public init(patches patches: Tensor<Double>, labels labels: Tensor<Int32>, given parameters: HyperParameters? = nil) {
    ...
    // print("sizes", H_, W_, C_)
    // training data shape [600, 40, 70, 1]
    ...
    // print("Shape of imagebatch", imageBatch.shape)
    // print("Shape of imagebatch", imageBatch.unstacked().count)
    ...
    for (epochIndex, epoch) in epochs.prefix(epochCount).enumerated() {
      ...
      // print("encoder 1", model.encoder1)
      // print("encoder 2", model.encoder2)
      // print("encoder 3", model.encoder3)
      for batchSamples in epoch {
        // print(".")
        let batch = batchSamples.collated
        // let batch = Tensor<Double>(stacking: batchSamples.map { $0.frame!.patch(at: $0.obb) })
        // let type = [Int32](batchSamples.map { $0.type == TrackingLikelihoodModel<PretrainedDenseRAE, MultivariateGaussian, GaussianNB>.PatchType.bg ? 0 : 1})
        // print("..")
...
public struct SmallerNNClassifier: Layer{
  ...
  /// Initialize  given an image batch
  public init(patches patches: Tensor<Double>, labels labels: Tensor<Int32>, given latentDimension: Int? = nil) {
    print("init from image batch")
    let (H_, W_, C_) = (patches.shape[1], patches.shape[2], 1)
...
public struct NNClassifierLoss {
  ...
  public func callAsFunction(
    ...
    let totalLoss = softmaxCrossEntropy(logits: output, labels: imageBatch.label)
    return totalLoss
  }

  @differentiable
  public func callAsFunction(
    _ model: LargerNNClassifier, _ imageBatch: BeeBatch, printLoss: Bool = false
  ) -> Tensor<Double> {
    let batchSize = imageBatch.patch.shape[0]
    let output = model(imageBatch.patch)
    let totalLoss = softmaxCrossEntropy(logits: output, labels: imageBatch.label)
    ...
    @differentiable
  public func callAsFunction(
    _ model: SmallerNNClassifier, _ imageBatch: BeeBatch, printLoss: Bool = false
  ) -> Tensor<Double> {
    let batchSize = imageBatch.patch.shape[0]
    let output = model(imageBatch.patch)
    let totalLoss = softmaxCrossEntropy(logits: output, labels: imageBatch.label)
...
extension SmallerNNClassifier : Classifier {}
extension LargerNNClassifier: Classifier {}
...
public struct PretrainedNNClassifier : Classifier{
  ...
  public init(patches patches: Tensor<Double>, labels labels: Tensor<Int32>, given: HyperParameters?) {
    inner = NNClassifier(
