...
///
/// If there are `n` columns, and if each column is a linear map from `A` to `B`, then `Matrix<Columns>` is the linear map from
/// `A^n` to `B` obtained by applying each column to its corresponding element of `A^n` and summing the results.
///
...
/// )
/// ```
/// We view each `Vector3d` as a linear map from `ScalarVector<Double>` to `Vector3d` via scalar multiplication.
/// So `m` is a linear map from `ScalarVector<Double>^2` (aka `Vector2d`) to `Vector3d`.
/// `m(Vector2d(1, 2))` multiplies the first column by `1`, multiplies the second column by `2`, and sums the results, to get `Vector3d(1, 2, 3)`.
...
extension Matrix: LinearMap where Columns: CollectionOfLinearMaps {
  ...
  ///
  /// This is a collection of column inputs because the matrix linear map works by applying each column to the corresponding element of the input.
  public typealias Input = Columns.CollectionOfElementInputs
  
  /// The output of the linear map.
  ///
  /// This is the same as the column output because the matrix linear map works by summing outputs from each column.
  ...
  public func callAsFunction(_ x: Input) -> Output {
    ...
    // Accumulate the rest of the result by applying subsequent columns to subsequent input components.
  ...
  ///
  /// This applies `self` to each column of `other` and stacks the results horizontally into a result matrix.
  ///
  ...
  where Matrix<OtherColumns>.Column == Input,
...
///
/// This protocol allows us to determine the input type of a matrix. For example, the input type of `Matrix<Array3<Vector2f>>` is `Vector3f`.
public protocol CollectionOfLinearMaps: Collection where Element: LinearMap {
  /// The collection of element inputs.
  associatedtype CollectionOfElementInputs: Collection & Vector where CollectionOfElementInputs.Element == Element.Input
}
