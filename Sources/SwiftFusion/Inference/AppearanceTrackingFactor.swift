...
public struct AppearanceTrackingFactor<LatentCode: FixedSizeVector>: LinearizableFactor2 {
  ...
  /// The size, in `(height, width)` pixels of the target in `measurement`.
  ...
  ///   - targetSize: the size, in `(height, width)` pixels of the target in `measurement`. If this
  ///                 is not specified, then the target size is assumed to be the same size a the
  ///                 patches generated by `appearanceModel`.
  public init(
