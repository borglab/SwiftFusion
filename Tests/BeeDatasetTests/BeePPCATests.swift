...
final class BeePPCATests: XCTestCase {
  ...
  func testPPCATracking() {
    ...
    fg.store(AppearanceTrackingFactor(poseId, latentId, measurement: frames[1].mean(alongAxes: [2]), appearanceModel: ppca.decodeWithJacobian, dimensionReducer: ppca.Ut!))
